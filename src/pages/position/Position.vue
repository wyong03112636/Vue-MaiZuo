  ...
  <div class="wrap-position">
    ...
    <van-index-bar :index-list="indexList" highlight-color="#ff5f16" v-else>
      <section>
        ...
        <p>
          <span v-for="city in hot_city" :key="city.id" @click="handlePickCity(city.id, city.nm)">{{
            city.nm
          }}</span>
        </p>
      </section>
        ...
        <div v-for="(city, key) in cityList" :key="key">
          ...
          <div class="city-list">
            <div
              ...
              v-for="c in city"
              :key="c.id"
              @click="handlePickCity(c.id, c.nm)"
            >
              {{ c.nm }}
            </div>
          </div>
...
export default {
  ...
  created() {
    this.indexListInit = ['定位', '热门'];
  },

  computed: {
    indexList() {
      return [...this.indexListInit, ...Object.keys(this.cityList)];
    },
  },
  ...
  async mounted() {
    ...
    let reducedResult = Object.keys(groupResult)
      .sort()
      .reduce((obj, key) => {
